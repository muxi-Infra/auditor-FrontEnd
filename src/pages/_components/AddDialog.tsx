...
export function AddDialog({
  ...
  const handleGetMembers = () => {
    getAllUsers()
      .then((res) => {
        setMembers(
          res.filter((user) => !projectMembersIds?.includes(user.id as number))
        );
      })
      .catch((err) => {
        console.log(err);
  ...
  const handleSearch = (value: string) => {
    selectUser(value, api_key)
      .then((response) => {
        ...
        setMembers(
          response.filter(
            (user) => !projectMembersIds?.includes(user.id as number)
          )
        );
      })
    ...
    <div className="flex items-center justify-center">
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogTrigger>
          ...
          >
            <img className="mr-1 h-4 w-4" src="/plus2.png" alt="" />
            <p>新增</p>
          </StatusButton>
        </DialogTrigger>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              <SearchInput
                action={(value) => handleSearch(value)}
                placeholder="search"
                className="h-12 rounded-md border-none text-[#E6864E] placeholder-[#E6864E]"
              ></SearchInput>
            </DialogTitle>
            <DialogDescription className="h-[360px] overflow-auto">
              <div className="ml-6 mt-4 flex flex-col gap-2">
                {members.map((member) => {
                  return (
                    <UserCard
                      checked={addMembers.some((i) => {
                        return i.user_id === member.id;
                      })}
                      onToogle={() => handleAdd(member.id as number)}
                      key={member.id}
          ...
          <DialogFooter className="flex flex-row justify-end gap-10">
            <DialogClose asChild>
              <StatusButton variant="cancel" className="shadow-none">
                取消
              </StatusButton>
            </DialogClose>
            <StatusButton
